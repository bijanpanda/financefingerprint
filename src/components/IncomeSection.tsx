  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden border border-slate-100 shadow-sm">
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-3.5 border-l-[3px] border-l-[var(--fill-success)] hover:bg-slate-50 transition-colors"
      >
        <div className="flex items-center gap-2.5">
          <svg className="w-4 h-4 text-teal-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m0 0l-4-4m4 4l4-4" />
          </svg>
          <span className="text-[15px] font-semibold text-teal-700">Income</span>
        ...
        <div className="px-3 pb-3">
          ...
          <div className="space-y-0.5">
            {items.map((item) => (
              <LineItemRow
                ...
                currency={currency} itemType="income"
          ...
          {addOpen ? (
            <div className="flex items-center gap-2 mt-2">
              <input
                type="text" placeholder="Income item name..." value={newName}
                onChange={(e) => setNewName(e.target.value)}
                onKeyDown={(e) => { if (e.key === "Enter") handleAdd(); if (e.key === "Escape") setAddOpen(false); }}
                autoFocus
                className="flex-1 px-3 py-2 text-sm border border-teal-300 rounded-lg bg-[var(--surface-1)] focus:ring-2 focus:ring-teal-400 focus:border-transparent outline-none"
              />
              <button onClick={handleAdd} className="px-4 py-2 text-sm bg-[var(--fill-success)] text-white rounded-lg font-medium">Add</button>
            ...
            <button
              ...
              className="mt-2 w-full flex items-center gap-2 px-3 py-2.5 rounded-lg border border-dashed border-slate-200 text-sm text-slate-400 hover:border-teal-300 hover:text-teal-600 hover:bg-teal-50/50 transition-colors"
            >

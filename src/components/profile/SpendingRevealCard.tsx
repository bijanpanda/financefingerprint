...
import { SpendingArchetypeDefinition, SpendingPersona } from "@/types/profile";

interface Props {
  spending: SpendingPersona;
  definition: SpendingArchetypeDefinition;
}

export default function SpendingRevealCard({ spending, definition }: Props) {
  return (
    <div className="glass-card rounded-2xl p-8 text-center animate-fadeIn">
      ...
      <div className="mb-4">
        ...
        <p className="text-amber-600 font-medium mt-1">{definition.tagline}</p>
      </div>

      {/* Composite score */}
      <div className="inline-flex items-center gap-2 bg-amber-50 rounded-full px-4 py-2 mb-6">
        <span className="text-sm text-gray-600">Spending Score:</span>
      ...
      <div className="grid grid-cols-2 gap-3 mb-8 max-w-md mx-auto">
        {[
          { label: "Impulse Control", value: spending.dimensionScores.impulseControl, weight: "30%" },
          { label: "Savings Consistency", value: spending.dimensionScores.savingsConsistency, weight: "25%" },
          { label: "Planning Behaviour", value: spending.dimensionScores.planningBehaviour, weight: "20%" },
          { label: "Lifestyle Inflation", value: spending.dimensionScores.lifestyleInflation, weight: "15%" },
          { label: "Emotional Spending", value: spending.dimensionScores.emotionalSpending, weight: "10%" },
        ].map((dim) => (

...
interface FamilyOrphanedEmailProps {
  firstName: string;
  initiatorName: string;
  familyUrl: string;
}

export function FamilyOrphanedEmail({ firstName, initiatorName, familyUrl }: FamilyOrphanedEmailProps) {
  return (
    <Html>
      <Head />
      <Preview>{initiatorName} is no longer part of your family on Vestrofin</Preview>
        ...
        <Container style={{ maxWidth: "560px", margin: "0 auto", backgroundColor: "#ffffff", borderRadius: "16px", overflow: "hidden", boxShadow: "0 4px 24px rgba(0,0,0,0.08)" }}>
          ...
          <Section style={{ padding: "36px 40px 0" }}>
            <div style={{ textAlign: "center", marginBottom: "24px" }}>
              <div style={{ width: "64px", height: "64px", borderRadius: "50%", background: "#fffbeb", margin: "0 auto 16px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <span style={{ fontSize: "32px" }}>💡</span>
              </div>
              <Heading style={{ color: "#1e293b", fontSize: "22px", fontWeight: "bold", margin: "0 0 8px" }}>
                Your family needs your confirmation
              </Heading>
              <Text style={{ color: "#64748b", fontSize: "15px", lineHeight: "1.7", margin: 0 }}>
                Hi {firstName}, <strong>{initiatorName}</strong> is no longer part of your family group on
                Vestrofin, and there&apos;s no other adult (Principal) left in it. Your budget data is safe and
                unchanged — you just need to confirm to continue as an independent account whenever you&apos;re
                ready.
              </Text>
            </div>

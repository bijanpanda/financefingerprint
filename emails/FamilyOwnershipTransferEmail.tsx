...
interface FamilyOwnershipTransferEmailProps {
  firstName: string;
  previousInitiatorName: string;
}

export function FamilyOwnershipTransferEmail({ firstName, previousInitiatorName }: FamilyOwnershipTransferEmailProps) {
  return (
    <Html>
      <Head />
      <Preview>You&apos;re now the organizer of your family on Vestrofin</Preview>
          ...
          <Section style={{ padding: "36px 40px 0" }}>
            <div style={{ textAlign: "center", marginBottom: "24px" }}>
              ...
              <Heading style={{ color: "#1e293b", fontSize: "22px", fontWeight: "bold", margin: "0 0 8px" }}>
                You&apos;re now the family organizer
              </Heading>
              <Text style={{ color: "#64748b", fontSize: "15px", lineHeight: "1.7", margin: 0 }}>
                Hi {firstName}, <strong>{previousInitiatorName}</strong> has left the family group on Vestrofin,
                so you&apos;re now its organizer. You can invite new members and manage the family from here on.
              </Text>
            </div>
